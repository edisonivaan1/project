...
const Game: React.FC = () => {
  ...
  const [_isKeyboardMode, setIsKeyboardMode] = useState(false); // Indica si el usuario está usando navegación por teclado
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (currentQuestion.isDragAndDrop && !isAnswerSubmitted) {
        ...
        if (event.shiftKey && !event.ctrlKey && !event.altKey && !event.metaKey) {
          // const _key = event.key; // Variable no utilizada
        ...
        if (!event.ctrlKey && !event.shiftKey && !event.altKey && !event.metaKey && (selectedWordIndex !== null || selectedSpaceIndex !== null)) {
          ...
          if (_key >= '1' && _key <= '2') {
            const targetSpace = parseInt(_key) - 1; // Convertir 1-2 a índices 0-1
            event.preventDefault();
            event.stopPropagation();
